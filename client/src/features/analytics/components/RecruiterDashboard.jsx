        ...
        <Card className="bg-white">
          ...
          {applications.total === 0 ? (
            ...
            <div className="space-y-3">
              {[
                { label: 'Applied', count: applications.byStatus['APPLIED'] || 0, color: 'bg-gray-400' },
                { label: 'Screening', count: applications.byStatus['SCREENING'] || 0, color: 'bg-blue-400' },
                { label: 'Shortlisted', count: applications.shortlisted || 0, color: 'bg-yellow-400' },
                { label: 'Interviews', count: applications.interviews || 0, color: 'bg-purple-400' },
                { label: 'Selected', count: applications.selected || 0, color: 'bg-green-500' },
              ].map(stat => (
                <div key={stat.label}>
          ...
          <div className="mt-6 flex justify-between items-center">
            <span className="text-sm text-gray-500">
              <span className="text-red-500 font-medium">{applications.rejected || 0}</span> rejected
            </span>
            <Link to="/jobs" className="text-blue-600 hover:text-blue-800 text-sm font-medium">
              Manage Vacancies &rarr;
            </Link>
          </div>
        </Card>
          ...
          <div className="grid grid-cols-1 gap-3">
            <Link to="/jobs" className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50 transition-colors">
              <div className="flex items-center">
                ...
                <div>
                  <h4 className="font-medium text-gray-900">Post a New Vacancy</h4>
                  <p className="text-sm text-gray-500">Create a job listing to attract candidates.</p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="bg-white">
          <div className="flex justify-between items-center mb-4 border-b pb-2">
            <h3 className="text-lg font-bold text-gray-900">Recent Vacancies</h3>
            <Link to="/jobs" className="text-xs text-blue-600 hover:text-blue-800 font-medium">
              View All ({jobs.total}) &rarr;
            </Link>
          </div>
          {(!jobs.recent || jobs.recent.length === 0) ? (
            ...
            <div className="space-y-3">
              {jobs.recent.map((job) => (
                <div key={job._id} className="flex justify-between items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
                  <div>
                    <h4 className="font-semibold text-gray-900 text-sm">{job.title}</h4>
                    <p className="text-xs text-gray-500">{job.employmentType} &bull; {job.workplaceType}</p>
                  </div>
        ...
        <Card className="bg-white">
          <div className="flex justify-between items-center mb-4 border-b pb-2">
            ...
            <Link to="/applications" className="text-xs text-blue-600 hover:text-blue-800 font-medium">
              View All ({applications.total}) &rarr;
            </Link>
          </div>
          {(!applications.recent || applications.recent.length === 0) ? (
            ...
            <div className="space-y-3">
              {applications.recent.map((app) => (
                <div key={app._id} className="flex justify-between items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
                  <div>
                    <h4 className="font-semibold text-gray-900 text-sm">{app.studentId?.name || 'Applicant'}</h4>
                    <p className="text-xs text-gray-500">Applied for: {app.jobId?.title || 'Vacancy'}</p>
                  </div>
